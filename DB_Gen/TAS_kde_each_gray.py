...
def TAS_each(filename = 'GeoRoc_Database.db',tag = 'VOL'):    
            ...
            if 35 <= center_x <= 80 and 0 <= center_y <= 17.6478:
                ...
                if(data_amount>=964):          
                    ...
                    if os.path.exists(file_path):
                        ...
                        # 计算数据点的概率密度
                        # 将概率场KDE保存为文件
                        os.makedirs(tag + '_GMM_kde', exist_ok=True)                            
                        # Save the KDE object to a file
                        with open(tag + '_GMM_kde/'+label+'_kde.pkl', 'wb') as f:
                            pickle.dump(kde, f)
                    ...
                    # 绘制等高线
                    contour = ax.contour(x_grid, y_grid, prob_grid, cmap='gray_r')
                    ...
                    # # ax.scatter(x, y, c=probs, label=label, cmap='gray', edgecolors='none',alpha = alpha)
                    ...
                    print(f"{label} Data amount is {data_amount}, Time taken: {time_taken:.3f} seconds")
                    ...
                    # 计算在视域范围内的数据点的数量
                    visible_points = tag_df[(tag_df['Type'] == label) &
                                            (tag_df['SIO2_wt_calibred'] >= xlim[0]) & 
                    ...
                    # 在图上显示可见的数据点的数量
                    ax.text(0.05, 0.95, f'Used points: {len(x)}', transform=ax.transAxes, verticalalignment='top')

                    fig.tight_layout()
                    # 保存图，包含图例
                    # 创建存图的文件夹
                    # os.makedirs(tag + '_GMM_svg_gray', exist_ok=True)
                    os.makedirs(tag + '_GMM_pdf_gray', exist_ok=True)
                    os.makedirs(tag + '_GMM_jpg_gray', exist_ok=True)
                    # fig.savefig(tag+'_GMM_svg_gray/'+ label +'_gray.svg')
                    fig.savefig(tag+'_GMM_pdf_gray/'+ label +'_gray.pdf')
                    fig.savefig(tag+'_GMM_jpg_gray/'+ label +'_gray.jpg', dpi=600)
                    # plt.show()
    ...
    pd.DataFrame(result_list).to_csv('TAS_test_prob_'+tag+'.csv',index=False)
    conn.close()
