...
def TAS_each(filename = 'Corrected/Remove_LOI_GeoRoc.db', rock_type = 'VOL',output_dir='TAS'):
            ...
            if 35 <= center_x <= 80 and 0 <= center_y <= 17.6478:
                ...
                if(data_amount>964):
                    try: 
                        ...
                        # print(f"Data amount is {data_amount} and Alpha is {alpha:.3f}")
                        ...
                        # 创建一个网格
                        x_grid, y_grid = np.meshgrid(np.linspace(min(x), max(x), 64), np.linspace(min(y), max(y), 64))
                        ...
                        # 将每个概率密度除以总和，进行归一化
                        # prob_grid = prob_grid / total_prob
    ...
    # 保存图，包含图例
    # 创建存图的文件夹
    os.makedirs(rock_type + '_GMS_svg', exist_ok=True)
    os.makedirs(rock_type + '_GMS_pdf', exist_ok=True)
    os.makedirs(rock_type + '_GMS_jpg', exist_ok=True)
    # fig.savefig('TAS_figure_' + tag + '_KDE.svg')
    fig.savefig(rock_type +'_GMS_svg/TAS_figure_' + rock_type+ '_KDE.svg')
    fig.savefig(rock_type +'_GMS_pdf/TAS_figure_' + rock_type+ '_KDE.pdf')
    fig.savefig(rock_type +'_GMS_jpg/TAS_figure_' + rock_type+ '_KDE.jpg', dpi=600)
