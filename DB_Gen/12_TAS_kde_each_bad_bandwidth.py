...
def TAS_silver(filename = 'Corrected/Remove_LOI_GeoRoc.db', rock_type = 'VOL',output_dir='TAS'):
    ...
    for label, group in grouped:
        ...
        # 删除包含NaN的行
        df = pd.DataFrame({"SIO2(WT%)": x, "ALL_Alkaline(WT%)": y})
        df = df.dropna()

        x = df["SIO2(WT%)"]
        y = df["ALL_Alkaline(WT%)"]
        center_x = x.mean()
        center_y = y.mean()
        ...
        if label not in labelled_groups:
            if 35 <= center_x <= 80 and 0 <= center_y <= 17.6478:
                ...
                if(data_amount>964):          
                    ...
                    ax.set_title(r"TAS Diagram with Silverman bandwidth", fontsize=9)
                    ax.set_xlim(35,80)
                    ...
                    visible_points = df[(df["SIO2(WT%)"] >= xlim[0]) & 
                        (df["SIO2(WT%)"] <= xlim[1]) & 
                        (df["ALL_Alkaline(WT%)"] >= ylim[0]) & 
                        (df["ALL_Alkaline(WT%)"] <= ylim[1])]


                    num_visible_points = len(visible_points)

                    # 在图上显示可见的数据点的数量
                    ax.text(0.8, 0.98, f'Visible points: {num_visible_points}', transform=ax.transAxes, verticalalignment='top')
                   
                    fig.tight_layout()
                    # 保存图，包含图例
                    # 创建存图的文件夹
                    os.makedirs(rock_type + '_GM_silver_svg', exist_ok=True)
                    os.makedirs(rock_type + '_GM_silver_pdf', exist_ok=True)
                    os.makedirs(rock_type + '_GM_silver_jpg', exist_ok=True)
                    fig.savefig(rock_type + '_GM_silver_svg/'+ label +'_silver.svg')
                    fig.savefig(rock_type + '_GM_silver_pdf/'+ label +'_silver.pdf')
                    fig.savefig(rock_type + '_GM_silver_jpg/'+ label +'_silver.jpg', dpi=600)
                    # plt.show()
...
def TAS_median(filename = 'Corrected/Remove_LOI_GeoRoc.db', rock_type = 'VOL',output_dir='TAS'):
        ...
        if label not in labelled_groups:
            if 35 <= center_x <= 80 and 0 <= center_y <= 17.6478:
                ...
                if(data_amount>964):          
                    ...
                    ax.set_xlabel(r"$SiO_2$", fontsize=9)
                    ax.set_ylabel(r"$Na_2O+K_2O$", fontsize=9)
                    ax.set_title(r"TAS Diagram with median bandwidth", fontsize=9)
                    ax.set_xlim(35,80)
                    ax.set_ylim(0,17.647826086956513)  
                    ...
                    # 保存图，包含图例
                    # 创建存图的文件夹
                    os.makedirs(rock_type + '_GM_median_svg', exist_ok=True)
                    os.makedirs(rock_type + '_GM_median_pdf', exist_ok=True)
                    os.makedirs(rock_type + '_GM_median_jpg', exist_ok=True)
                    fig.savefig(rock_type + '_GM_median_svg/'+ label +'_median.svg')
                    fig.savefig(rock_type + '_GM_median_pdf/'+ label +'_median.pdf')
                    fig.savefig(rock_type + '_GM_median_jpg/'+ label +'_median.jpg', dpi=600)
                    # plt.show()
...
TAS_silver('Corrected/Remove_LOI_GeoRoc.db','VOL')
TAS_silver('Corrected/Remove_LOI_GeoRoc.db','PLU')
TAS_median('Corrected/Remove_LOI_GeoRoc.db','VOL')
TAS_median('Corrected/Remove_LOI_GeoRoc.db','PLU')