...
def TAS_original(filename = 'Corrected/Remove_LOI_GeoRoc.db',rock_type = 'VOL',output_dir='TAS'):
    ...
    if os.path.exists(output_dir+'/'+'TAS_Base_' + rock_type + '_original.pkl'):
        ...
        ax.set_xlabel(r"$SiO_2$", fontsize=14)
        ax.set_ylabel(r"$Na_2O+K_2O$", fontsize=14)
        ax.set_title(r"TAS Diagram Classical", fontsize=14)
        ax.set_xlim(35,80)
        # ax.set_ylim(0,17.647826086956513)  
        # 在 y=17.647826086956513 的位置画一条横线
        ax.axhline(17.647826086956513, linestyle='-', color='black', linewidth=0.3)
    ...
    # 保存图，包含图例
    # 创建存图的文件夹
    fig.savefig(output_dir+'/'+'TAS_Base_' + rock_type + '_original.svg')
    # fig.savefig(output_dir+'/'+'TAS_Base_' + rock_type + '_original.pdf')
    fig.savefig(output_dir+'/'+'TAS_Base_' + rock_type + '_original.jpg', dpi=600)
...
def TAS_new(filename = 'Corrected/Remove_LOI_GeoRoc.db',rock_type = 'VOL',output_dir='TAS'):
    ...
    if os.path.exists(output_dir+'/'+'TAS_Base_' + rock_type + '.pkl'):
        ...
        ax.set_xlabel(r"$SiO_2$", fontsize=14)
        ax.set_ylabel(r"$Na_2O+K_2O$", fontsize=14)
        ax.set_title(r"TAS Diagram with Adaptive Bandwidth Gaussian KDE", fontsize=14)
        ax.set_xlim(35,80)
        # ax.set_ylim(0,17.647826086956513)  
        # 在 y=17.647826086956513 的位置画一条横线
        ax.axhline(17.647826086956513, linestyle='-', color='black', linewidth=0.3)
...
def TAS_No_Lines(filename = 'Corrected/Remove_LOI_GeoRoc.db',rock_type = 'VOL',output_dir='TAS'):
    ...
    # 检查是否存在'TAS_Base_' + tag + '_Nolines.pkl'文件
...
def TAS_No_Colors(filename = 'Corrected/Remove_LOI_GeoRoc.db',rock_type = 'VOL',output_dir='TAS'):
    ...
    # 检查是否存在'TAS_Base_' + tag + '_No_Colors.pkl'文件
    if os.path.exists(output_dir+'/'+'TAS_Base_' + rock_type + '_No_Colors.pkl'):
        # 如果存在，从文件中读取tag_color_dict
        with open(output_dir+'/'+'TAS_Base_' + rock_type + '_No_Colors.pkl', 'rb') as f:
            fig = pickle.load(f)
    else:
        pass
        ...
        fig.tight_layout()        
        # fig.set_size_inches(10 , 10)
        with open(output_dir+'/'+'TAS_Base_' + rock_type + '_No_Colors.pkl', 'wb') as f:
            pickle.dump(fig, f)
...
TAS_new(filename, 'VOL', output_dir = 'TAS')
TAS_new(filename, 'PLU', output_dir = 'TAS')
...
TAS_original(filename, 'VOL', output_dir = 'TAS')
TAS_original(filename, 'PLU', output_dir = 'TAS')
